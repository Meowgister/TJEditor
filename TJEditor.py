...
class Cursor:  # This stores variables related to the "cursor", a position in the file that is used to draw notes
    ...
    def __init__(self, line_pos, note_pos, hori_pos, vert_pos, note_centre, note_value, note_ending, current_image,
                 current_directory, key_pressed, music_pos, current_bpm, bpm_list, bpm_list_pos, lines, first_line_1,
                 first_line_2, first_line_3, first_line_4, first_line_5):
...
def initialise_objects(filename, filepath, current_bpm, offset, lines):
    ...
    cursor = Cursor(0, 0, 0, 0, (0, 0), 12, "ths", "temp", filepath, 0, offset, current_bpm, [], 0, lines, 0, 0, 0, 0,
                    0)
...
def refresh_window(song_data, cursor, win):
    ...
    text = create_text(song_data, "Note value: " + str(48 // cursor.get_note_value()) + cursor.get_note_ending(), True,
                       (255, 255, 255))
...
def scroll_left(song_data, cursor, win):
    ...
    if re.match("[#][START]", str(lines[cursor.get_line_pos() - 1])) or re.match("[#][START]",
                                                                                 str(lines[cursor.get_line_pos()])):
...
def place_note(song_data, cursor, win):
    ...
    if str(current_line[cursor.get_note_pos()]) == str(
            cursor.get_key_pressed()) or cursor.get_key_pressed() == 0:  # If note is placed or note = none
        ...
        current_line = str(current_line[0:cursor.get_note_pos()] + str(cursor.get_key_pressed()) + current_line[
                                                                                                   cursor.get_note_pos() + 1:])
